...
def generateData(mat_file, img_directory):
	...
	for folder in range(5):  #!!! changed from 100 to 10
		...
		for filename in glob.glob(path):
			...
			if img.shape[0]<=300 and img.shape[1]<=300:
				...
				images_and_labels.append([data_x, data_y, np.argmax(data_y)/10])
	...
	autoencoder_train_y = list(final[:, 2][:-test_size])
	autoencoder_test_y = list(final[:, 2][-test_size:])
	...
	'''
	return train_x, train_y, test_x, test_y, autoencoder_test_y
